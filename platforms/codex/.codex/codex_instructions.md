...
### config.toml is the Master Config (NOT legacy codex.config.json)
This scaffold targets the **modern Codex CLI config format**: `.codex/config.toml`
(TOML, schema-validated via `#:schema https://developers.openai.com/codex/config-schema.json`).
Older gald3r notes referenced a legacy `codex.config.json` + `suggest`/`auto-edit`/`full-auto`
mode naming; those are **superseded** by `config.toml` with `approval_policy` / `sandbox_mode`
keys. Do not create a `codex.config.json` -- it is not the active config surface. See
`PLATFORM_SPEC.md` for the full version note.
...
> NOTE: This list is kept in sync with the `[[skills.config]]` blocks in `config.toml`
> (the authoritative deployed registration). If you add a skill folder under
> `.codex/skills/`, you MUST add a matching `[[skills.config]]` block -- registering a
> path that does not exist on disk can cause a startup error.
...
### MCP Support
Codex CLI supports Model Context Protocol servers, **declared in `config.toml`** (modern format,
not the legacy `codex.config.json` `mcpServers` block). This scaffold does not declare any MCP
servers by default. The exact `config.toml` table key for MCP servers is not yet verified against
the official schema -- confirm against `https://developers.openai.com/codex/config-schema.json`
before adding servers. See `PLATFORM_SPEC.md` §8.
